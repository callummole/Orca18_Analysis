...
def simulate_offsets(course, yr_array, onset_time, sabs, smooth = True):
	...
	for yr in sabs:
		...
		Car.autofile_i = 0 #0 reserved for balanced
		Cars.append(Car)
	return Cars
...
def full_course():
	...
	L = 16#2sec.
	myStraight  = simTrackMaker.lineStraight(startpos = [0,0], length= L)#, texturefile='strong_edge_soft.bmp')

	#Create Bend
	myrads = 80
	myBend = simTrackMaker.lineBend(startpos = myStraight.RoadEnd, rads = myrads, x_dir = 1, road_width=3.0) 

	bend2 = simTrackMaker.lineBend(startpos = myBend.RoadEnd, rads = myrads, x_dir = -1, z_dir = -1, road_width=3.0)
	bend3= simTrackMaker.lineBend(startpos = bend2.RoadEnd, rads = myrads, x_dir = 1, z_dir = 1, road_width=3.0)
	bend4 = simTrackMaker.lineBend(startpos = bend3.RoadEnd, rads = myrads, x_dir = -1, z_dir = -1, road_width=3.0)

	#midline and edges
	Course_RoadStart = myStraight.RoadStart
	Course_midline = np.vstack((myStraight.midline, myBend.midline, bend2.midline, bend3.midline, bend4.midline))	
	Course_CurveOrigin = myBend.CurveOrigin

	"""
	plt.plot(myStraight.midline[:,0],myStraight.midline[:,1], alpha = .5)
	plt.plot(myBend.midline[:,0],myBend.midline[:,1], alpha = .5)
	plt.plot(bend2.midline[:,0],bend2.midline[:,1], alpha = .5)
	plt.plot(bend3.midline[:,0]+4,bend3.midline[:,1], alpha = .5)
	plt.plot(bend4.midline[:,0]+4,bend4.midline[:,1], alpha = .5)
	plt.show()
	"""

		
	#Temp HACK to store in list while I improve trackmaker.
	print(Course_RoadStart)	
	Course = [Course_RoadStart,Course_midline, Course_CurveOrigin]

	return Course

course_full = full_course()
...
yawrate_readout_full = playbackdata.get("YawRate_seconds").values
#yawrate_readout = yawrate_readout[mask]
world_z = playbackdata.get("World_z").values
yawrate_readout_bend = yawrate_readout_full[world_z>= 16]
#midline = np.array([track['midlinex'].values, track['midlinez'].values]).T

course_bend = [(0,0), midline, (80,0)]

Cars_ideal_bal = simulate_offsets(course_bend, bend_yr_array, .5, sabs = [-5.72957795, -1.19868047, -0.52191351, -0.3039716], smooth = False)
...
Cars_ideal_sobol = simulate_offsets(course_bend, bend_yr_array, .5, sabs = sabs, smooth = False)
...
axes[0,0].set_xlabel('Time from Failure to Lane Exit (s)', fontsize = 10)
...
axes[0,1].set_xlabel('Time from Failure to Lane Exit (s)', fontsize = 10)
...
for file in sobol_list:
	...
	readout_full = playbackdata.get("YawRate_seconds").values
	#yawrate_readout = yawrate_readout[mask]
	sobol_YRs.append(readout_full)
	world_z = playbackdata.get("World_z").values
	readout = readout_full[world_z>= 16]	
	sobol_YRs_bend.append(readout)
...
fig_cm = np.array([8.7,10])
fig_inc = fig_cm /2.54 

fig, axes = plt.subplots(2,1, figsize = fig_inc, constrained_layout = True)


axes[0].axhline(y = -1.5, color = (.7, .7, .7), linestyle = '--')
axes[0].axhline(y = 1.5, color = (.7, .7, .7), linestyle = '--')
...
for sab, ons, afile in zip(sabs, onsets, autofile_idxs):
	...
	Car, _ = tsim.runSimulation(course_full, yr_readout, 80, sab, ons)	
	Car.autofile_i = afile+1 #0 reserved for balanced	
	#print(Car.pos_history[][0])
...
for car in Cars_sobol:	
	...
	print(len(sb)/60)
	print(sb[-1])
	axes[0].plot((np.arange(0, len(sb)) / 60), sb, color = 'xkcd:light grey')

	axes[0].plot((len(sb)-1)/60 , sb[-1], color = 'xkcd:light grey', marker = ".", markersize = 5)


	yr = car.yawrate_history

	axes[1].plot((np.arange(0, len(yr)) / 60), np.rad2deg(yr), color = 'xkcd:light grey')	

	axes[1].plot((len(yr)-1)/60 , np.rad2deg(yr[-1]), color = 'xkcd:light grey', marker = ".", markersize = 5)

print("BAL")
Cars_bal = simulate_offsets(course_full, yawrate_readout_full, 6, sabs = [-5.72957795, -1.19868047, -0.52191351, -0.3039716])

for car, col in zip(Cars_bal, rgbas):
	sb = car.error_history	
	axes[0].plot((np.arange(0, len(sb)) / 60), sb, color = col)

	axes[0].plot((len(sb)-1)/60 , sb[-1], color = col, marker = ".", markersize = 5)

	yr = car.yawrate_history
	axes[1].plot( (np.arange(0, len(yr)) / 60), np.rad2deg(yr), color = col)
	
	axes[1].plot((len(yr)-1)/60 , np.rad2deg(yr[-1]), color = col, marker = ".", markersize = 5)

axes[0].set_xlabel('Time into Trial (s)', fontsize = 10)
axes[0].set_ylabel('Lane Position (m)', fontsize = 10)
axes[0].axvline(x = 15, color = 'k', linestyle = '-')
axes[0].set_xlim([2, 17])
axes[0].invert_yaxis()
axes[0].set_yticks([-1.5, 0, 1.5])  
axes[0].set_ylim([1.7,-1.7])  

axes[1].axhline(y =np.rad2deg(8.0 / 80), color = (.1, .1, .1), linestyle = ':')
axes[1].set_xlabel('Time into Trial (s)', fontsize = 10)
axes[1].set_ylabel('Yaw rate ($^\circ$/s)', fontsize = 10)
axes[1].axvline(x = 15, color = 'k', linestyle = '-')
axes[1].set_xlim([2, 17])

for a, label in zip(axes.flat, ['A','B']):
...
legend_elements = [Line2D([0],[0],color = rgbas[0], lw = 1, label = "2.2 s"),
					Line2D([0],[0],color = rgbas[1], lw = 1, label = "4.7 s"),
					Line2D([0],[0],color = rgbas[2], lw = 1, label = "7.1 s"),
					Line2D([0],[0],color = rgbas[3], lw = 1, label = "9.6 s"),
				Line2D([0],[0], color = "xkcd:light grey", lw = 1, label = "2.9 s - 19.5 s")] 


#axes[0].legend(handles = legend_elements, loc = [1,-.5], fontsize = 8, frameon = False, title = r'$TTLC_F$', title_fontsize = 8, labelspacing = .2)
axes[0].legend(handles = legend_elements, loc = [.01,.06], fontsize = 7, frameon = False, labelspacing = .12)
	

#annotations
axes[1].annotate('Bend YR', xy=(16, np.rad2deg(8.0 / 80)), xytext=(15.25, np.rad2deg(8.0 / 80)+1.5), fontsize = 8,
            arrowprops=dict(arrowstyle="->",
                            connectionstyle="arc3"),
            )

axes[1].annotate('Trial\nEnd', xy=(15, np.rad2deg(8.0 / 80)-4.5), xytext=(12, np.rad2deg(8.0 / 80)-6), fontsize = 8,
            arrowprops=dict(arrowstyle="->",
                            connectionstyle="arc3"),
            )
...
#add fully simulated balanced cars.
cars = []
for car in Cars_bal:
	#+2 to onset time because we do not include the straight bit in the above plots
	cars.append([car.yawrateoffset, car.onsettime, car.autofile_i, car.simulated_ttlc])
...
sim_res = pd.DataFrame(cars, columns = ['sab','onsettime','autofile_i', 'simulated_ttlc'])
sim_res.to_csv('simulated_ttlcs.csv')

#save as csv the simulated ttlcs
