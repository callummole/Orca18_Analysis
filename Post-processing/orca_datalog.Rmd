...
loadsteergazedata <- function(){
  ...
  calc_TTC <- function(SB_mirrored, SB_change){
    ...
    distance_to_edges <- road_edges - SB_mirrored
    TTLCs <- distance_to_edges / (SB_change * 60) #whichever road edge is opposite the direction of travel will give negative TTC
    TTLC <- apply(TTLC, 1, max)
    return(TTLC)
...
calc_TTC <- function(SB_mirrored, SB_change){
  ...
  #need to incorporate the proximity to road edges and direction of travel.
  SB_mirrored <- array(SB_mirrored)
  #SB_change <- array(SB_change)
...
trialdata<- steergazedata %>% 
  filter(trialid == trial)

trialdata <- trialdata %>% 
  mutate(TTLC_new = calc_TTC(SB_mirrored, SB_change) )
