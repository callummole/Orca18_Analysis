...
ggplot(data = filter(takeover_trialavgs, RT>0, failure_type != "Benign"), aes(x = RT, y = SWA_max, col = factor(failure_type), fill = factor(failure_type))) +
  geom_point(alpha = .8) +
  #facet_wrap(~failure_type) +
  scale_colour_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type")  +
  scale_fill_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type")  +
...
```{r plot TTLC, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15,fig.height=8,fig.cap="Fig 11. TTLC."}
...
ggplot(data = filter(takeover_state, TTLC < 10, RT > 0), aes(x = sb_mirrored, y=RT, col = factor(failure_type))) + 
  geom_point() +
  scale_colour_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type") 


ggplot(data = filter(takeover_state, TTLC < 10, RT > 0), aes(x = TTLC, y=RT, col = factor(failure_type))) + 
  geom_point() +
  scale_colour_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type") 
