...
def sim_ttlc(onset, autofile_i, sab, rt):

    filename_list = ["Midline_80_1.csv","Midline_80_2.csv","Midline_80_3.csv","Midline_80_4.csv","Midline_80_5.csv"]
    file = filename_list[autofile_i]   
    playbackdata = pd.read_csv("Data//"+file) 	
    yawrate_readout = playbackdata.get("YawRate_seconds")
    Course = load_track() 
    myrads = 80
    _, onset_ttlc, rt_ttlc = runSimulation(Course, yawrate_readout, myrads, sab, onset, rt)
