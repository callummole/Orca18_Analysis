...
track = pd.read_csv("../Data/track_with_edges_orca_80.csv.csv")
...
viewpoint = (0, 16) #end of straight
yaw = 0      
...
def plot_projection_contours(ax):
	...
	xpog, zpog =  dp.angles_to_world(coords)
...
def plot_projection_estimation():	
	...
	for i, a in enumerate(angles):
		...
		if i % 2 == 0: 
			al = 1		
			ax.plot(a[0], a[1], mec = 'k', mfc =color_dict[a[0]], marker = marker_dict[a[1]], ms = 4, alpha = al)
		else:
			ax.plot(a[0], a[1], mec = color_dict[a[0]], mfc = 'xkcd:pale', marker = marker_dict[a[1]], ms = 4, alpha = al)
	...
	#add points
	left, bottom, width, height = [0.4, 0.6, 0.5, 0.3]
	ax2 = fig.add_axes([left, bottom, width, height])
	ax2.set_xlim(xmin = -12, xmax = 0)
	ax2.set_ylim(ymin = 0, ymax = 30)
	
	V = np.linspace(-15, -.3, 500)
	Hs = [np.zeros(len(V)), np.ones(len(V)) * 35]	
	...
	for i, h in enumerate(Hs):
		...
		ax2.plot(V, TH, color = color_dict[h[0]])		
	...
	for i, a in enumerate(angles):
		...
		if i % 2 == 0: 
			addintersectinglines(ax2, V, TH, th, color_dict[a[0]], 1)

			ax2.plot(a[1], th, mec = 'k',  mfc=color_dict[a[0]], marker = marker_dict[a[1]], ms = 4, zorder = 3)
			
		else:
			addintersectinglines(ax2, V, TH, th, color_dict[a[0]], 1)
			ax2.plot(a[1], th, mec = color_dict[a[0]],  mfc='xkcd:pale', marker = marker_dict[a[1]], ms = 4, zorder = 3)
			
		
		ths.append(th[0])
	
	xticks = [x for x in range(-10,0)]
	yticks = [y for y in range(5, 30, 5)]
	ax2.set_xticks(xticks)
	ax2.set_xticklabels(xticks, fontsize = 8)
	ax2.set_yticks(yticks)    
	ax2.set_yticklabels(yticks, fontsize = 8)
	ax2.set_xlabel("Vertical Angle ($^\circ$)", fontsize = 10)
	ax2.set_ylabel("Time Headway (s)", fontsize = 10)
...
def plot_track():
	...
	for t in target_circles:
		...
		plt.plot(t[:,1], t[:,0], color = (.4,.4,.4), markersize = .2)

	
	plt.vlines(x = 20, ymin = -30, ymax = -20, color = 'b', linestyle = '-')
	plt.vlines(x = -20, ymin = -30, ymax = -20, color = 'g', linestyle = '-')
	plt.vlines(x = 20, ymin = 30, ymax = 20, color = 'g', linestyle = '-')
	plt.vlines(x = -20, ymin = 30, ymax = 20, color = 'b', linestyle = '-')
...
#plot_projection_contours()
#plot_midline_estimation()
#plot_track()
