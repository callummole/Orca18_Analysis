...
#dodge <- position_dodge(width=0.1)  
p_var <- ggplot(data = var_RTs, aes(x =  1/ttlc_onset, y= log(std), colour = cogload, group = cogload)) +
  geom_point(alpha = .2)
...
p_var_dist <- ggplot(data = var_RTs, aes(x = std, fill = cogload, group = cogload)) +
...
print(ribbon)
#ggsave("overall_predictiveintervals_ttlc_noload.png", plot = ribbon, dev = "png", width = 18, height = 8.5, units = "cm", dpi = 300, type = "cairo")
...
ribbon <- ggplot(preds, aes(x = ttlc_onset)) +
  ...
  #geom_line(aes(y = Q0.5, col = cogload), alpha = .4, linetype = 2) +
  ...
  scale_color_manual(values = cogload_cols, guide = guide_legend(reverse = TRUE)) + scale_fill_manual(values = cogload_cols) + guides(fill = FALSE) +
  theme_classic() +
  xlab(expression("TTLC"["Failure"]*" (s)")) + ylab(expression("TTLC"["Takeover"]*" (s)")) +
...
pars = c("b_Intercept", "b_ttlc_onset", "b_cogloadload", "b_ttlc_onset:cogloadload", "b_sigma_Intercept", "b_sigma_ttlc_onset",
               "b_sigma_cogloadload", "b_sigma_ttlc_onset:cogloadload")


breaks = c("b_I","b_to", "b_cl", "b_to:cl", "b_sig_I", "b_sig_to",
               "b_sig_cl", "b_sig_to:cl")
...
draws_coef <- MOD %>% 
  ...
  group_by(b) %>% 
  mode_hdi() %>% 
  mutate(b = fct_relevel(b, "b_Intercept", "b_ttlc_onset", "b_cogloadload", "b_ttlc_onset:cogloadload", "b_sigma_Intercept", "b_sigma_ttlc_onset",               "b_sigma_cogloadload", "b_sigma_ttlc_onset:cogloadload"))
...
effects_grid <- plot_grid(ribbon, forest, ncol = 2, labels = c("A","B"), label_size = 12, rel_widths = c(1.5,1))
ggsave("ttlc_effects.png", plot = effects_grid, dev = "png", width = 18, height = 6, units = "cm", dpi = 300, type = "cairo")
```
...
###### population means without random effects ##############
...
pc_avg <- sim_avg_pcross(map_params, nsamples = 1000)
pc <- pc_avg
#sims = 1000
...
for (row in 1:nrow(samples)) {
  ...
  print(row)
  simrow <- samples[row,]
  p_c <- sim_avg_pcross(simrow, 1000)
