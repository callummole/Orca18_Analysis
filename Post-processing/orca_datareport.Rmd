...
Fig 3 shows massive differences in how quickly drivers disengage the automation after SAB onset (using positive RTs only). In the _Benign_ condition drivers disengaged the system in `r round(as.double(disengage_perc) * 100)` % of trials. Since drivers react at such different speeds across the three failure types the failure type conditions will be look at in turn.
...
Fig 7 shows some interesting difference. Sudden failures clearly produce more 'going out of lane', but they people generally take-over at lower SBs when the failure is Sudden. _note that the Benign averages out because the take-over is either side of zero).
