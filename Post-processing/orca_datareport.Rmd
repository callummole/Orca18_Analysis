...
This document is a report for on the pre-registered experiment https://osf.io/s6vam/. The experiment examined silent failures in a highly controlled setting. Each trial starts with automated steering. In some trials a bias is introduced that causes the vehicle to veer off the road either suddenly (within 1.5 s) or gradually (~ 4 s). The bias is to yaw-rate, so introduces error by causing the vehicle to understeer or oversteer. Effectively, the manipulation biases the mapping between steering angle - which does not change - and yaw-rate, which does change. Therefore we will henceforth call this error _Steering Angle Bias_ (SAB). Participants are required to keep within the road edges and intervene if they feel that it is necessary. They complete the task without distraction or with a easy or difficult distraction. There are also two bend radii used: sharp (40 m) or gradual (80 m).
...
Fig 3 shows massive differences in how quickly drivers disengage the automation after the SAB (using positive RTs only). In the _None_ condition drivers disengaged the system in `r round(as.double(disengage_perc) * 100)` % of trials. Since drivers react at such different speeds across the three failure types the failure type conditions will be look at in turn.
...
```{r plotting RT takeover within radius, echo=FALSE, message=FALSE, warning=FALSE, fig.width=14,fig.height=4.5,fig.cap="Fig 4. RT takeover across cognitive loads"}
...
Here we analyse the driver response after taking over control. Firstly we plot some trajectories to understand the constraints the driver is placed under. Remeber that there are 6 potential automation trajectories, with onset times of the SAB varying between 5 and 9. This variability makes for messy plots if plotting them all in cartesian space. Therefore, Fig 6 fixes the onset time (=5 s) and the bend radius (=40 m). You can see that the potential automation trajectories are all pretty similar. It is also clear that the _Sudden_ take-overs are tightly bunched compared to the _Gradual_ and _None_ (see Fig 4). _None_ is a misnomer because there is actually an offset introduced in these trials but the offset isn't large enough to leave the road. Therefore, participants might sense some drift in lane position, prompting them to take-over. Note that in the _Gradual_ panel there is one trial where the take-over happens before the SAB is introduced. This happens in `r round(as.double(premature_perc) * 100)` % of trials.
...
By plotting lane position after take-over we can compare the trajectories across different onset times and pre-recorded trajectories. 


```{r plotting lane position, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15,fig.height=5,fig.cap="Fig 7. Steering bias for different failure types"}
...
#plot steering bias
p_sb <- ggplot(data = filter(steergaze_manual, f < 500), aes(x = f, y= SB_mirrored, col = factor(failure_type))) +
  geom_path(aes(group = trialid), alpha = .1) +
...
Fig 7 shows some interesting difference. Sudden failures clearly produce more 'going out of lane', but they people generally take-over at lower SBs when the failure is Sudden. _note that the none averages out because the take-over is either side of zero).
...
#plot steering bias
p_sb_cogload <- ggplot(data = filter(steergaze_manual, f < 300), aes(x = f, y= SB_mirrored, col = factor(cogload))) +
  geom_path(aes(group = trialid), alpha = .1) +
  geom_smooth(se= F, size = 1.5) +
  facet_wrap(~failure_type) +
...
```
Cogload seems to correct for errors more gradually when cognitively loaded. We see this also in Wilkie et al. 2019.


```{r plotting radii within failure types, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15,fig.height=5,fig.cap="Fig 9. Steering bias for Radii within failure types"}
...
#plot steering bias
p_sb_radii <- ggplot(data = filter(steergaze_manual, f < 300), aes(x = f, y= SB_mirrored, col = factor(radii))) +
  geom_path(aes(group = trialid), alpha = .1) +
  geom_smooth(se= F, size = 1.5) +
  facet_wrap(~failure_type) +
  scale_colour_manual(values = wes_palette("Darjeeling1",n=2), name = "Bend Radius") +
...
```
Fig 9. Take longer to correct in 80 m rather than 40 m, on average.

Let's have a look at the SWA plots.



```{r plotting SWA, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15,fig.height=5,fig.cap="Fig 10. SWA for cogload within different failure types"}
...
#plot steering bias
p_swa_cogload <- ggplot(data = filter(steergaze_manual, f < 300), aes(x = f, y= SWA_mirrored, col = factor(cogload))) +
  geom_path(aes(group = trialid), alpha = .1) +
  geom_smooth(se= F, size = 1.5) +
  facet_wrap(~failure_type) +
  scale_colour_manual(values = wes_palette("Cavalcanti1",n=3), name = "Cognitive Load") +
...
```
Here we see capping of the SWA for Sudden failures, where the desired turning input is greater than 90 degrees. Essentially we hit a limit of the wheel system. Participants are likely to then over-correct because they over-turn the wheel. This should be corrected for is a future design. 

However, we would expect the participants to not turn the wheel as much as they need to (essentially, to not correct fully) for the cognitively loaded conditions. This is contaminated here by the sensitivity capping. 

```{r filtering takeover trials, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10,fig.height=10,fig.cap="Fig 5. A) Average trajectories, B) Shown as steering bias"}
...
p_sb <- ggplot(data = filter(steergaze_manual, radii == 40, failure_type != "None", f < 300), aes(x = f, y= SB_mirrored, col = factor(failure_type))) +
  geom_path(aes(group = trialid), alpha = .3) +
  geom_smooth(se= F) +
  #scale_colour_manual(values = wes_palette("Cavalcanti1",n=3), name = "Cognitive Load") +
...
ggplot(data = filter(takeover_state, ttlc < 10, RT > 0, failure_type != "None", rads == 40), aes(x = RT, y= ttlc, shape = factor(autofile), col = factor(onset))) + 
  geom_point(alpha = .5)

ggplot(data = filter(pre_takeover_state, ttlc < 10, RT > 0, failure_type != "None"), aes(x = ttlc, y= RT)) + 
  geom_point(alpha = .5)
