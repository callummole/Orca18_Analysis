...
This document is a report for on the first run of the pre-registration https://osf.io/s6vam/. The purpose of this report is to give a coherent description of the data so that design decisions for follow-on studies can be traced. The experiment examines silent failures in a highly controlled setting. Each trial starts with automated steering. In some trials a bias is introduced that causes the vehicle to veer off the road either suddenly (within 1.5 s) or gradually (~ 4 s). Participants are required to keep within the road edges and intervene if they feel that it is necessary. They complete the steering task on two bend radii, sharp (40 m) or gradual (80 m), without distraction or with an easy or difficult distraction.
...
Fig 1 plots two indices of cognitive performance for the cognitive task when the driver was not steering: _Percentage Correct_ (PC, True positives + True negatives / total letters heard); _Reaction Time_ (RT, reaction time for True positives).

```{r plot Cognitive Task Performance, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10.5,fig.height=4.5,fig.cap="Fig 1. Cognitive Task Performance. A) Percentage Correct. B) Reaction Time. C-D) Absolute Count Error"}
...
p_report <- plot_grid(p_rt + theme(legend.position="none"), 
              p_pc + theme(legend.position="none"), 
              labels = c("A", "B"), nrow=1)
p_report <- plot_grid(p_report, legend, rel_widths = c(4,.5))
show(p_report)


```
 
Fig 1 shows that people respond differently to each Cognitive Load condition. Participants react slower to heard target letters if they are listening for three targets rather than only one target (Fig 2A). Participants also experienced greater difficulty in deciding the appropriate response when there were three targets as compared to when there was only one (Fig 2B). The next section will examine how this gets reflected in steering measures.

### Driver Takeover.

The pre-registration specifies two hypotheses relating to taking over control of the vehicle:

-	__H1__: Participants will react slower to silent failures as cognitive load increases (i.e. more time will have elapsed from the steering angle bias to the driver disengaging the automation).
...
- RT from SAB onset to disengagement of the system via a button press (can be negative if they take over before the SAB is introduced). This measure will be referred to as RT~takeover~.

- RT from SAB onset to first steering movement (RT~steer~).

These two measures are expected to correlate, and deciding on criteria for the first steering movement is non-trivial, so for the purpose of this brief report only RT~takeover~ is reported. 
...
```{r plotting RT takeover, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10.5,fig.height=4.5,fig.cap="Fig 2. RT takeover across failure types"}
...
Fig 2 shows massive differences in how quickly drivers disengage the automation after SAB onset (using positive RTs only). In the _Benign_ condition drivers disengaged the system in `r round(as.double(disengage_perc) * 100)` % of trials. Since drivers react at such different speeds (therefore have very different scales) across the three failure types the failure type conditions will be plotted separately.

```{r plotting RT takeover within failure type conditions, echo=FALSE, message=FALSE, warning=FALSE, fig.width=14,fig.height=4.5,fig.cap="Fig 3. RT takeover across cognitive loads"}
...
Fig 3 shows cognitive load differences in RT~takeover~ within failure type (using positive RTs only, collapsed across bend radii). For Sudden failures (leftmost in Fig 3) there are clear difference between None, Easy, and Hard. People switch to manual mode quickest when not performing a distractor task; they switch slowest when the distractor task is more difficult. However, note that effect size is small: the peaks are only about .1s apart (modes are `r mode_sudden_none` s, `r mode_sudden_easy` s, and `r mode_sudden_hard` s respectively) with SDs of approximately .2s. This difference is less pronounced for Gradual failures (middle) and Benign failures (right). In the Gradual Failure and Benign failure conditions the data is spread out over a wider range than in Sudden failures. Nevertheless, for _Gradual_ failures the median RT for No load seems to be earlier (`r round(median_gradual_none, 2)` s) than the _Easy_ (`r round(median_gradual_easy, 2)` s) and _Hard_ (`r round(median_gradual_hard, 2)` s) cognitive load conditions.
...
__H1 Summary__: Reaction time to silent failures does indeed increase as cognitive load increases, though this effect is small and depends on the severity of the failure.
...
```{r plotting trajectories, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15,fig.height=8,fig.cap="Fig 6. Trajectories with a fixed onset time (at 5 seconds) and for bend radius of 40 m. Blue star is the time of onset. Dots are the time of take-over"}
...
```{r plot SWA variability and RT, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15,fig.height=8,fig.cap="Fig 10. Steering aggression measures correlated with RT. A) Maximum Steering Wheel Angle, B) Standard Deviation of Steering Wheel Angle, C) Average steering wheel velocity"}
...
#ttlc by RT
#ggplot(data = filter(takeover_state, TTLC < 10, RT > 0), aes(x = TTLC, y=RT, col = factor(cogload))) + 
#  geom_point() +
#  scale_colour_manual(values = wes_palette("Cavalcanti1",n=3), name = "Cog Load") 

p_sb_RT <- ggplot(data = filter(takeover_state, TTLC < 10, RT > 0, failure_type != "Benign"), aes(x = sb_mirrored, y=RT, col = factor(failure_type))) + 
  geom_point() +
  scale_colour_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type") +
  theme_transition_report +
...
p_sb_ttlc <- ggplot(data = filter(takeover_state, TTLC < 10, RT > 0, failure_type != "Benign"), aes(x = TTLC, y=RT, col = factor(failure_type))) + 
  geom_point() +
  scale_colour_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type") +
  theme_transition_report +
...
However, the magnitude of steering corrections are not solely determined by how quickly drivers respond. Error develops at different rates in different conditions, so an identical RT~takeover~ across both _Gradual_ and _Sudden_ failure types results in different lane positions (Fig 11A) and urgency (Fig 11B) at takeover. Therefore, if one plots SWA~max~ against RT~takeover~, stratified by failure type, there are clear differences between the conditions (Fig 12A). If steering magnitude was driven solely by speed of response, we would expect both failure types to 'collapse' along the same dimension. Fig 12B (SWA~max~ by lane position and Fig 12C (SWA~max~ by TTLC) seem to be closer to achieving a single dimension which determines response across conditions, yet there is still reasonable stratification across failure type conditions. 
...
p_SWAmax_RT <- ggplot(data = filter(takeover_state, RT>0, failure_type != "Benign"), aes(x = RT, y = SWA_max, col = factor(failure_type), fill = factor(failure_type))) +
  geom_point(alpha = .8) +
  #facet_wrap(~failure_type) +
  scale_colour_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type")  +
  scale_fill_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type")  +
...
p_SWAmax_sb <- ggplot(data = filter(takeover_state, RT>0, TTLC< 10, failure_type != "Benign"), aes(x = sb_mirrored, y = SWA_max, col = factor(failure_type), fill = factor(failure_type))) +
  geom_point(alpha = .8) +
  #facet_wrap(~failure_type) +
  scale_colour_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type")  +
  scale_fill_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type")  +
...
p_SWAmax_ttlc <- ggplot(data = filter(takeover_state, RT>0, TTLC< 10, failure_type != "Benign"), aes(x = TTLC, y = SWA_max, col = factor(failure_type), fill = factor(failure_type))) +
  geom_point(alpha = .8) +
  #facet_wrap(~failure_type) +
  scale_colour_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type")  +
  scale_fill_manual(values = rev(wes_palette("BottleRocket2",n=3)), name = "Failure Type")  +
...
With this in mind we proceed to conduct regression analysis with variables from the scene as continuous predictors, rather than with failure_type as categorical predictors. In the first instance the RT~takeover~ is predicted from the state of the world at .25 s before the switch to manual mode (to allow for execution delays). Two variables are chosen for a first pass: lane position and change in lane position.
