...
simulate_pcross <- function(samples, tt = c(0,.05,.1,.15,.2,.25,.3,.35,.4,.45,.5)){
  time <- Sys.time()
...
#prcros
#what was the minimum ttlc that didn't exit the road? 
...
samps <- samples[1:500,] #only a subset..
pc_full <- simulate_pcross(samps)
...
ggplot(pc_bounds_full, aes(x = ttlc_onset, y= Estimate, col = factor(tminus), group = factor(tminus))) +
  geom_line() +
  facet_wrap(~cogload) +
  coord_cartesian(xlim = c(.5,10), ylim = c(0,.5))

ggsave("average_pcross_tminus.png", plot = last_plot(), dev = "png", width = 13.2, height = 6, units = "cm", dpi = 300, type = "cairo")
...
psim <- ggplot() +
 ...
 # geom_line(data = toplot, aes(x = ttlc_onset, y =pcross, col = cogload, group = interaction(pp,cogload)), inherit.aes = FALSE, alpha = .2) +
  ...
  scale_y_continuous(breaks = c(0, 0.025, 0.05, 0.075, .1), labels = c("0",".025",".05",".075",".1")) +
