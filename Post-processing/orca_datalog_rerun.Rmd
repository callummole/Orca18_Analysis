---
title: 'Silent Failures in Automation RERUN. Pilot dataset of 2 participants'
author: "Callum Mole"
output:
  html_document:
    df_print: paged
...
I report on 2 participants who piloted our Silent Failures paradigm. Half the experiment was factorial: two cognitive loads (NoLoad, Load) and four factored failure severities (_Steering Angle Bias_; SAB). In the other half of the experiment the failure severities and onset times were quasi-random so that there was a spread of different types.
...
steergazedata <- read_csv("collated_steering.csv")  
...
levels(steergazedata$cogload)[1]<- "noload"
levels(steergazedata$cogload)[2] <- "load"
...
ggplot(filter(steergaze_trialavgs, design == "balanced"), aes(x = RT, group = factor(sab), fill = factor(sab))) +
  ...
  xlim(c(0,10)) +
  scale_fill_manual(values = rev(wes_palette("BottleRocket2",n=4)), name = "Failure Type") +
  theme_transition_report +
  xlab(expression("RT"["takeover"]*" (s)"))

```

 In total, drivers disengaged the system in `r round(as.double(disengage_perc) * 100)` % of trials. 
